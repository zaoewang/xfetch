...
export function init(intiOptions) {
    const { apis, config, beforeRequest = () => true, requestSuccessed = () => true, requestFailed = () => { }, getHeaders = () => ({}), getLoading = () => ({
        showLoading: () => { },
        hideLoading: () => { }
    }), formatResponse } = intiOptions;
        ...
        return async (data = {}, options = {}) => {
            ...
            try {
                ...
                if (responseFormatType) {
                    if (responseFormatType == 'source')
                        return response;
                    else
                        return response[responseFormatType]();
                }
                //最终输出的格式化
                if (formatResponse) {
    ...
    function getIntercept() {
        ...
        function addIntercept(path, method, data) {
            if (!data)
                data = {};
            let jdata = '{}';
            try {
        ...
        function removeIntercept(path, method, data) {
            if (!data)
                data = {};
            let jdata = '{}';
            try {
...
export async function $fetch(data, apiItem, options = {}) {
    apiItem.method = apiItem.method.toUpperCase();
    ...
    clearTimeout(timer);
    return response;
    function formatRequestBody(data, method, contentType) {
